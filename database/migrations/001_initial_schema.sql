...
CREATE TABLE IF NOT EXISTS resources.distributions (
    ...
    -- Indexes
    INDEX idx_distributions_user_id ON resources.distributions(user_id),
    INDEX idx_distributions_timestamp ON resources.distributions(timestamp),
    INDEX idx_distributions_type ON resources.distributions(distribution_type),
    INDEX idx_distributions_processed ON resources.distributions(processed)
);
...
CREATE TABLE IF NOT EXISTS resources.users (
    ...
    -- Indexes
    INDEX idx_users_user_id ON resources.users(user_id),
    INDEX idx_users_email ON resources.users(email),
    INDEX idx_users_status ON resources.users(status)
);
...
CREATE TABLE IF NOT EXISTS resources.community_pools (
    ...
    -- Indexes
    INDEX idx_community_pools_type ON resources.community_pools(pool_type),
    INDEX idx_community_pools_status ON resources.community_pools(status)
);
...
CREATE TABLE IF NOT EXISTS truth.channels (
    ...
    -- Indexes
    INDEX idx_truth_channels_channel_id ON truth.channels(channel_id),
    INDEX idx_truth_channels_type ON truth.channels(channel_type),
    INDEX idx_truth_channels_status ON truth.channels(status),
    INDEX idx_truth_channels_last_activity ON truth.channels(last_activity)
);
...
CREATE TABLE IF NOT EXISTS truth.messages (
    ...
    -- Indexes
    INDEX idx_truth_messages_channel_id ON truth.messages(channel_id),
    INDEX idx_truth_messages_type ON truth.messages(message_type),
    INDEX idx_truth_messages_status ON truth.messages(status),
    INDEX idx_truth_messages_created_at ON truth.messages(created_at)
);
...
CREATE TABLE IF NOT EXISTS mesh.nodes (
    ...
    -- Indexes
    INDEX idx_mesh_nodes_node_id ON mesh.nodes(node_id),
    INDEX idx_mesh_nodes_type ON mesh.nodes(node_type),
    INDEX idx_mesh_nodes_status ON mesh.nodes(status),
    INDEX idx_mesh_nodes_region ON mesh.nodes(region),
    INDEX idx_mesh_nodes_last_ping ON mesh.nodes(last_ping)
);
...
CREATE TABLE IF NOT EXISTS mesh.connections (
    ...
    -- Indexes
    INDEX idx_mesh_connections_source ON mesh.connections(source_node_id),
    INDEX idx_mesh_connections_target ON mesh.connections(target_node_id),
    INDEX idx_mesh_connections_type ON mesh.connections(connection_type),
    INDEX idx_mesh_connections_status ON mesh.connections(status),
    INDEX idx_mesh_connections_last_activity ON mesh.connections(last_activity),
...
CREATE TABLE IF NOT EXISTS analytics.daily_stats (
    ...
    -- Indexes
    INDEX idx_daily_stats_date ON analytics.daily_stats(date),
    UNIQUE(date)
);
...
CREATE TABLE IF NOT EXISTS analytics.performance_metrics (
    ...
    -- Indexes
    INDEX idx_performance_metrics_timestamp ON analytics.performance_metrics(timestamp),
    INDEX idx_performance_metrics_type ON analytics.performance_metrics(metric_type),
    INDEX idx_performance_metrics_component ON analytics.performance_metrics(component)
);
